...
In this case, Alice gets 2 + 7 = 9 piles in total. So we return 10 since it's larger.
...
public class Solution {
    public int stoneGameII(int[] piles) {
        ...
        // Compute the suffix sum for the piles
        suffixSum[n - 1] = piles[n - 1];
        for (int i = n - 2; i >= 0; i--) {
            suffixSum[i] = piles[i] + suffixSum[i + 1];
        }

        // Dynamic programming to find the maximum stones Alice can get
        for (int i = n - 1; i >= 0; i--) {
            for (int m = 1; m <= n; m++) {
                if (i + 2 * m >= n) {
                    dp[i][m] = suffixSum[i];
